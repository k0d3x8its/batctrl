# batctrl ⚡

[![CI](https://img.shields.io/github/actions/workflow/status/k0d3x8its/batctrl/ci.yml?style=flat&label=CI)](https://github.com/k0d3x8its/batctrl/actions/workflows/ci.yml) [![Latest Release](https://img.shields.io/github/v/release/k0d3x8its/batctrl?style=flat&label=Latest%20Release)](https://github.com/k0d3x8its/batctrl/releases/latest) [<img alt="TARS" src="https://img.shields.io/badge/TARS-dev-%23E84142.svg?logo=data:image/png;base64,">](https://github.com/k0d3x8its)

A lightweight Linux CLI utility to control battery charge limits (e.g. 60%–100%), persist settings across reboot, and restore state automatically using systemd.

Designed for systems that expose:
  -  `/sys/class/power_supply/BAT*/charge_control_end_threshold`

---

## 🚀 Features

- Set battery charge limit (60–100%, in increments of 10)
- Quick shortcuts:
  - `-c` / `--current` → show current limit
  - `-t` / `--toggle` → toggle between 80% and 100%
- Persistent state across reboot
- systemd service auto-restores last value
- Bash completion support
- Root-protected execution (requires sudo)

---

## ⚠️ Requirements

- Linux (kernel with charge control support)
- systemd
- sudo access
- Battery device exposed under `/sys/class/power_supply/` with `charge_control_end_threshold`

> Supported on most ThinkPads and many modern laptops. Verify support:
> ```bash
> ls /sys/class/power_supply/BAT*/charge_control_end_threshold
> ```

---

## 📦 Installation (Debian package)

Download the latest `.deb` from [Releases](https://github.com/k0d3x8its/batctrl/releases/latest), then install:

```bash
curl -LO https://github.com/k0d3x8its/batctrl/releases/latest/download/batctrl_v1.0.0_all.deb
sudo dpkg -i batctrl_v1.0.0_all.deb
```

Or with the GitHub CLI:

```bash
gh release download -R k0d3x8its/batctrl --pattern '*.deb'
sudo dpkg -i batctrl_v1.0.0_all.deb
```

Fix dependencies if needed:

```bash
sudo apt-get install -f
```

---

## 🧠 Usage

### Set battery limit

In increments of 10 (60–100):

```bash
sudo batctrl -60
sudo batctrl -70
sudo batctrl -80
sudo batctrl -90
sudo batctrl -100
```

### Show current limit

```bash
sudo batctrl --current
# or
sudo batctrl -c
```

### Toggle between 80% <---> 100%

```bash
sudo batctrl --toggle
# or
sudo batctrl -t
```

### Help

```bash
batctrl --help
```

---

## 🔁 Persistence

The last selected battery limit is stored in:
- `/var/lib/bat-limit/last_limit`

On reboot, a systemd service automatically restores it:
- `bat-limit.service`

---

## ⚙️ systemd Service

Installed at:
- `/etc/systemd/system/bat-limit.service`

Enabled automatically on install. Manage manually if needed:

```bash
sudo systemctl enable bat-limit.service
sudo systemctl start bat-limit.service
systemctl status bat-limit.service
```

---

## 📁 Project Structure

```bash
batctrl/
├── batctrl_pkg/              # Debian package source tree
│   ├── DEBIAN/
│   │   ├── control
│   │   ├── postinst
│   │   └── prerm
│   ├── usr/local/bin/
│   │   └── batctrl           # main CLI script
│   └── etc/bash_completion.d/
│       └── batctrl
├── .github/workflows/
│   └── release.yml           # tag-triggered .deb build + draft release
├── CHANGELOG.md
└── README.md
```

---

## 📜 License

See [LICENSE](./LICENSE).
